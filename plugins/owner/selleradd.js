exports.run = {
  ...
  async: async (m, {
     ...
     if (m.quoted) {
        if (m.quoted.isBot) return client.reply(m.chat, Func.texted('bold', `🚩 Can't make the bot a seller user.`), m)
        ...
        users.expiredseller += users.seller ? (86400000 * days) : ((new Date() * 1) + (86400000 * days))
        client.reply(m.chat, users.seller ? Func.texted('bold', `🚩 Succesfully added ${days} days seller access for @${jid.replace(/@.+/, '')}.`) : Func.texted('bold', `🚩 Successfully added @${jid.replace(/@.+/, '')} to seller user.`), m).then(() => users.seller = true)
