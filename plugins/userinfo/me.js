exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         caption += `	◦  *Expired Premium* : ${user.expired == 0 ? '-' : Func.timeReverse(user.expired - new Date() * 1)}\n`
         caption += `   ◦  *Seller* : ${(user.seller ? '√' : '×')}\n`
         caption += `   ◦  *Expired Seller* : ${user.expiredseller == 0 ? '-' : Func.timeReverse(user.expiredseller - new Date() * 1)}\n\n`
